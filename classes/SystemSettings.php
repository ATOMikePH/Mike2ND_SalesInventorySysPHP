...
class SystemSettings extends DBConnection{
	...
	function update_settings_info(){
		...
		if (isset($_FILES['logo']) && $_FILES['logo']['tmp_name'] != '') {
			$logo = 'uploads/logo-' . (time()) . '.png';
			$dir_path = base_app . $logo;
			$upload = $_FILES['logo']['tmp_name'];
			$type = mime_content_type($upload);
			$allowed = array('image/png', 'image/jpeg');
	
			if (!in_array($type, $allowed)) {
				$resp['msg'] .= " But Logo image failed to upload due to an invalid file type.";
			} else {
				$new_height = 200;
				$new_width = 200;
				...
				if ($gdImg) {
					...
					$resp['msg'] .= " But Logo image failed to upload due to an unknown reason.";
				}
			}
	
			if ($uploaded_img === true) {
				if (isset($_SESSION['system_info']['logo'])) {
					$qry = $this->conn->query("UPDATE system_info SET meta_value = '{$logo}' WHERE meta_field = 'logo' ");
					if (is_file(base_app . $_SESSION['system_info']['logo'])) unlink(base_app . $_SESSION['system_info']['logo']);
				} else {
					$qry = $this->conn->query("INSERT INTO system_info SET meta_value = '{$logo}', meta_field = 'logo' ");
				}
			}
		}
	
		if (isset($_FILES['cover']) && $_FILES['cover']['tmp_name'] != '') {
			$cover = 'uploads/cover-' . time() . '.png';
			$dir_path = base_app . $cover;
			$upload = $_FILES['cover']['tmp_name'];
			$type = mime_content_type($upload);
			$allowed = array('image/png', 'image/jpeg');
	
			if (!in_array($type, $allowed)) {
				$resp['msg'] .= " But Cover image failed to upload due to an invalid file type.";
			} else {
				$new_height = 720;
				$new_width = 1280;
				...
				if ($gdImg) {
					...
					$resp['msg'] .= " But Cover image failed to upload due to an unknown reason.";
				}
			}
	
			if ($uploaded_img === true) {
				if (isset($_SESSION['system_info']['cover'])) {
					$qry = $this->conn->query("UPDATE system_info SET meta_value = '{$cover}' WHERE meta_field = 'cover' ");
					if (is_file(base_app . $_SESSION['system_info']['cover'])) unlink(base_app . $_SESSION['system_info']['cover']);
				} else {
					$qry = $this->conn->query("INSERT INTO system_info SET meta_value = '{$cover}', meta_field = 'cover' ");
				}
			}
		}
