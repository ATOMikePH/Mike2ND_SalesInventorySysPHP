...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		foreach($_POST as $k => $v){
			if(in_array($k,array('firstname','middlename','lastname','salutation','username','type'))){
				if(!empty($data)) $data .=" , ";
				$data .= " {$k} = '{$v}' ";
			}
		}
		...
		if($resp['status'] == 1){
			$data="";
			foreach($_POST as $k => $v){
				if(!in_array($k,array('id','firstname','middlename','lastname','salutation','username','password','type','confirm_password'))){
					if(!empty($data)) $data .=", ";
