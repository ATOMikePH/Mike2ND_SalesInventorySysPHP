...
// Query for top sales
$sql_sales = "SELECT il.name AS product_name, SUM(sl.total) AS total_sales
        FROM stock_list sl
        INNER JOIN item_list il ON sl.item_id = il.id
        WHERE sl.type = 2 {$date_condition}
        GROUP BY sl.item_id
...
if ($result_sales->num_rows > 0) {
    ...
    while ($row = $result_sales->fetch_assoc()) {
        ...
        $sales_table_data .= "<td>{$row['product_name']}</td>";
        $sales_table_data .= "<td>{$formatted_total_sales}</td>";
        $sales_table_data .= "</tr>";
    }
} else {
    $sales_table_data = "<tr><td colspan='3'>No sales data available.</td></tr>";
}
...
$sql_stocks = "SELECT il.name AS item_name, SUM(sl.quantity) AS total_quantity
        FROM stock_list sl
        INNER JOIN item_list il ON sl.item_id = il.id
        WHERE sl.type = 1 {$date_condition2}
        GROUP BY sl.item_id
...
if ($result_stocks->num_rows > 0) {
    ...
    while ($row = $result_stocks->fetch_assoc()) {
        ...
        $stocks_table_data .= "<td>{$row['item_name']}</td>";
