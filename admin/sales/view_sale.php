...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h4><i class="fas fa-money-bill-wave icon"></i> Sales Invoice - <?php echo $sales_code ?></h4>
    </div>
    ...
    <div><?php echo isset($input_by) ? $input_by : '' ?></div>
            ...
            <colgroup>
                ...
                <?php 
    $total = 0;
    $qry = $conn->query("SELECT s.*, i.name, i.sku, i.brand_id, b.name AS brand FROM `stock_list` s 
...
<td class="py-1 px-2 text-center"><?php echo number_format($row['quantity']) ?></td>
    ...
    <tr>
        <th class="text-center py-1 px-2" colspan="2"><strong>Mode of Payment</strong></th>
...
<?php endif; ?>
    ...
    <tr>
        <th class="text-right py-1 px-2" colspan="3">Less: VAT (<?php echo isset($tax_perc) ? $tax_perc : 0 ?>%)</th>
        <th class="text-right py-1 px-2 tax"><?php echo isset($tax) ? number_format($tax,2) : 0.00 ?></th>
    </tr>
    <tr>
        <th class="text-right py-1 px-2" colspan="3">Gross Amount: Net of VAT</th>
        <th class="text-right py-1 px-2 sub-total"><?php echo isset($total) ? number_format($total,2) : 0.00  ?></th>
    </tr>
    <tr>
    <th class="text-right py-1 px-2" colspan="1">VAT Amount (<?php echo isset($tax_perc) ? $tax_perc : 0 ?>%)</th>
        <th class="text-right py-1 px-2 tax"><?php echo  isset($tax) ? number_format($tax,2) : 0.00 ?></th>
    ...
    <tr>
        <th class="text-right py-1 px-2" colspan="1"><strong>Amount Tendered (PHP)</strong></th>
        ...
        <th class="text-right py-1 px-2 amount_tendered"><strong><?php echo  isset($amount) ? ($amount) : 0.00 ?></strong></th>
    </tr>
    <tr>
    <th class="text-right py-1 px-2" colspan="3">Change (PHP)</th>
    ...
    <a class="btn btn-flat btn-dark" href="<?php echo base_url.'/admin?page=sales' ?>"><i class="fas fa-backward"></i> Back To List</a>
    <button class="btn btn-flat btn-success" type="button" id="print"><i class="fas fa-print"></i> Print Receipt</button>
...
function calculateChange() {
   ...
   $(function () {
    $('#print').click(function () {
        ...
        _head.find('title').text("Sales Invoice Receipt- Print View");
        ...
        _el.append('<div class="receipt-container">' +
            ...
            '<div><hr></div>' +
            '<h4 class="text-center">Sales Invoice Receipt</h4>' +
            '</div><hr/>');

        var transactionDateTimeDiv = p.find('.col-md-6:contains("Transaction Date and Time")');
        ...
        _el.append('<div class="col-md-6 text-right">' +
            '<label class="control-label text-info">Transaction Date and Time</label>' +
            '<div>' + transactionDateTimeData + '</div>' +
            '</div>');

        
        var salesCodeDiv = p.find('.col-md-6:contains("Sales Transaction No.")');
    ...
    'margin-top: 5px;' + // Adjust this value as needed for spacing above the line
    'margin-bottom: 5px;' + // Adjust this value as needed for spacing below the line
    '}' +
    '}' +
    '</style>');
