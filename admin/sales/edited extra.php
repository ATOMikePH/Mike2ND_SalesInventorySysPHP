...
while ($row = $item->fetch_assoc()) {
    ...
    $item_arr[$row['id']] = $row;
    $cost_arr[$row['id']] = $row['price_cost'];
}

$i = 1;
$qry = $conn->query("SELECT i.*,c.name as category FROM `item_list` i inner join categories c on i.category_id = c.id order by `name` desc");
...
?>
    ...
    <?php foreach ($item_arr as $k => $v): ?>
        <option value="<?php echo $k; ?>">
            <?php echo $v['name'] . " - Available: " . number_format($v['available'] + $v['bbalance']); ?>
        </option>
    <?php endforeach; ?>
</select>
                            </div>
                ...
                <table class="table table-striped table-bordered" id="list">
                    ...
                    <tfoot>
                        ...
                        <tr>
                            ...
                            <th class="text-right py-1 px-2 discount text-success"><?php echo isset($discount) ? number_format($discount) : 0 ?></th>
                        </tr>
                        <tr>
                            ...
                            <th class="text-right py-1 px-2 tax text-warning"><?php echo isset($tax) ? number_format($tax) : 0 ?></th>
                        </tr>
                        <tr>
...
<script>
    ...
    $(function(){
        ...
        if (item && qty > 0 && unit && items[item] && items[item].available < qty) {
            alert_toast('Insufficient stock in the inventory.', 'error');
            return false;
        }
