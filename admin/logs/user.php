    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const fetchUserLogs = async () => {
                try {
                    const response = await fetch('fetch_user_logs.php'); // Adjust the path based on your file structure
            ...
            const displayLogs = (logs) => {
                ...
                if (logs.length === 0) {
                    ...
                    logs.forEach(log => {
                        ...
                        const dataName = `<span class="data-name">${log.data_name}</span>`;
                        const sectionName = `<span class="section-name">${getSectionName(log.type)}</span>`;

                        logEntry.innerHTML = `${datetime} : ${user} ${actionType} ${dataName} in ${sectionName}`;
